/**
 * leafletIcons.js
 * SVG inline helpers for Leaflet DivIcon — no React, no img tags, no encoding issues.
 * All paths extracted from lucide-react via renderToStaticMarkup.
 *
 * Usage:
 *   import { sciIncidentSvg, sciDepSvg, sciZoneSvg, resourceSvg } from '../utils/leafletIcons.js'
 *   html: `<div ...>${sciIncidentSvg('incendio', '#fb923c', 14)}</div>`
 */

// ── Raw SVG inner paths (Lucide, viewBox 0 0 24 24) ─────────────────────────
const P = {
...
/**
 * Returns an inline SVG string (no img tag, safe for L.divIcon html).
 * @param {string} iconName  — key in P above (Lucide component name)
 * @param {string} color     — CSS color e.g. '#fb923c'
 * @param {number} size      — pixel size (default 14)
 */
export function svgIcon(iconName, color, size = 14) {
  const paths = P[iconName]
  if (!paths) return ''
...
// ── Incident type → Lucide icon name ────────────────────────────────────────
const INCIDENT_LUCIDE = {
  sismo:             'Activity',
  colapso:           'House',
  incendio:          'Flame',
  vial:              'Car',
...
// ── Dependency key → Lucide icon name ───────────────────────────────────────
const DEP_LUCIDE = {
  ERUM:     'Ambulance',
  SSC:      'ShieldAlert',
  PC:       'Shield',
...
// ── Resource typeCode → Lucide icon name (grouped by VEHICLE TYPE, not institution) ──
const UNIT_LUCIDE = {
  // Ambulancias (todas las instituciones)
  'ambulancia-basica':    'Ambulance',
  'ambulancia-intensiva': 'Ambulance',
  'cruzroja-basica':      'Ambulance',
  'cruzroja-intensiva':   'Ambulance',
  // Rescate terrestre / personal en campo
  'rescate-erum':         'HardHat',
  'rescate-bomberos':     'HardHat',
  'pc-busqueda-rescate':  'HardHat',
  'ingenieros-sedena':    'HardHat',
  // Vehículos pesados / camiones (pipa, convoy)
  'pipa-bomberos':        'Truck',
  'convoy-sedena':        'Truck',
  // Vehículos de patrulla / mando
  'patrulla-ssc':         'Car',
  'pc-evaluacion':        'Car',
  // K9
  'k9-ssc':               'Dog',
  // Marina
  'marina-refuerzo':      'Ship',
  // Helicópteros
  'helicoptero-condor':   'Helicopter',
  'helicoptero-marina':   'Helicopter',
  // Instalaciones fijas
  'hospital-3er-nivel':   'Building2',
  'hospital-2do-nivel':   'Building2',
  'hospital-1er-nivel':   'Building2',
  'acv-fijo':             'HeartPulse',
  'acv-avanzado':         'HeartPulse',
}

// ── Zone type → Lucide icon name ────────────────────────────────────────────
const ZONE_LUCIDE = {
...
/** Inline SVG for a SCI incident type */
export function sciIncidentSvg(type, color, size = 14) {
  return svgIcon(INCIDENT_LUCIDE[type] ?? 'TriangleAlert', color, size)
}

/** Inline SVG for a SCI dependency */
export function sciDepSvg(dep, color, size = 14) {
  return svgIcon(DEP_LUCIDE[dep] ?? 'ShieldAlert', color, size)
}

/** Inline SVG for a resource unit typeCode */
export function sciUnitSvg(typeCode, color, size = 14) {
  return svgIcon(UNIT_LUCIDE[typeCode] ?? 'TriangleAlert', color, size)
}

/** Inline SVG for a SCI zone type */
export function sciZoneSvg(zone, color, size = 14) {
  return svgIcon(ZONE_LUCIDE[zone] ?? 'MapPin', color, size)
}

/** Inline SVG for brecha-mode resource markers (institutional colors) */
export const BRECHA_SVG = {
  hospital:   (c = '#93c5fd', s = 14) => svgIcon('Building2', c, s),  // blue-300 — professional
  fireStation:(c = '#fca5a5', s = 14) => svgIcon('Siren',     c, s),  // red-300 — emergency siren
  hypo:       (c = '#6ee7b7', s = 14) => svgIcon('Plus',      c, s),  // emerald-300 — hypothetical
}
